...
import (
	...
	istioapigatewayv1alpha1 "knative.dev/net-istio-api-gateway/pkg/client/listers/istioapigateway/v1alpha1"
...
// controller reconciling v1alpha1.Route.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Route. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Route) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Route.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Route. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Route) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.Route resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister istioapigatewayv1alpha1.RouteLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister istioapigatewayv1alpha1.RouteLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.Route, desired *v1alpha1.Route) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.IstioapigatewayV1alpha1().Routes(desired.Namespace)
		...
		updater := r.Client.IstioapigatewayV1alpha1().Routes(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Route) (*v1alpha1.Route, error) {
	...
	patcher := r.Client.IstioapigatewayV1alpha1().Routes(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Route) (*v1alpha1.Route, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Route, reconcileEvent reconciler.Event) (*v1alpha1.Route, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
